...
public class Checkout {
	...
	public void I_navigate_to_the_checkout_page_after_adding_items_to_the_basket()
			throws Throwable {
		...
		tescoMobileSimPDPPagePO
				.clickOnSearchByProductNameCatalogueNoOrKeywordTextField();
		tescoMobileSimPDPPagePO
				.typeInSearchByProductNameCatalogueNoOrKeywordTextField("M3E-C8KG");
		tescoMobileSimPDPPagePO.clickOnSearchButton();
		harryPotterDVDPDPPagePO = WC
				.getPageObject(HarryPotterDVDPDPPagePO.class);
	...
	public void the_Thank_you_tab_is_disabled_and_greyed_out() throws Throwable {
		...
		Assert.assertTrue(checkOutPagePO
				.checkThankYouTabIsDisabledAndGreyedOut());

	}

	@Given("^the delivery method - Collection is selected by default$")
	public void the_delivery_method_Collection_is_selected_by_default()
			throws Throwable {
	...
	public void the_following_elements_are_displayed(DataTable table)
			throws Throwable {
			...
			for (Map.Entry<String, String> entry : myValue.entrySet()) {
				...
				case "Item quantity X Item Name":
					result = checkOutPagePO
							.checkItemQuantityXItemNameIsDisplayed();
					Assert.assertTrue(result);
	...
	public void the_following_is_displayed_under_delivery_method_Collection(
	...
	public void I_navigate_to_the_checkout_page_after_adding_items_to_the_basket_as_a_logged_in_user(
			DataTable table) throws Throwable {
	...
	public void I_navigate_to_the_checkout_page_after_adding_items_to_the_basket_as_a_newly_registered_user()
			throws Throwable {
	...
	public void the_Pay_now_box_should_display_the_following(DataTable table)
			throws Throwable {
	...
	public void Collection_details_with_following_information_is_displayed(
			DataTable table) throws Throwable {
		...
		for (Map<String, String> myValue : mappedTable) {
			for (Map.Entry<String, String> entry : myValue.entrySet()) {
				...
				case "message to show when the item can be collected":
					result = checkOutPagePO
							.checkItemCollectionDateAndTimeIsDisplayed();
					Assert.assertTrue(result);
				...
				case "message to inform the customer that email would be sent and proof ID is required to be shown while collection":
					result = checkOutPagePO
							.checkItemCollectionInformationIsDisplayed();
					Assert.assertTrue(result);
				...
				case "price for collection as Free":
					result = checkOutPagePO
							.checkDeliveryCostIsDisplayedAsFree();
					Assert.assertTrue(result);
	...
	public void the_payment_card_details_with_the_following_is_displayed(
			DataTable table) throws Throwable {
		...
		for (Map<String, String> myValue : mappedTable) {
			for (Map.Entry<String, String> entry : myValue.entrySet()) {
				...
				case "expiry date":
					result = checkOutPagePO
							.checkExpiryDateDropDownIsDisplayed();
					Assert.assertTrue(result);
					LOG.debug("expiry date is displayed");
					break;
				case "security code":
					result = checkOutPagePO
							.checkSecurityCodeTextBoxIsDisplayed();
					Assert.assertTrue(result);
	...
	public void ewallet_section_displays_the_following(DataTable table)
			throws Throwable {
		...
		for (Map<String, String> myValue : mappedTable) {
			for (Map.Entry<String, String> entry : myValue.entrySet()) {
				...
				case "add clubcard vouchers":
					result = checkOutPagePO
							.checkAddClubcardVouchersIsDisplayed();
					Assert.assertTrue(result);
	...
	public void the_radio_button_of_only_the_first_store_is_selected() {
		...
		Assert.assertTrue(checkOutPagePO
				.checkOnlyFirstStoreRadioButtonIsSelected());
	...
	public void the_new_billing_address_is_added_on_the_checkout_page()
			throws Throwable {
