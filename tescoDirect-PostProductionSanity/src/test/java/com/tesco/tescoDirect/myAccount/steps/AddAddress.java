...
public class AddAddress {
	...
	public void I_am_taken_to_the_Add_a_new_address_page() throws Throwable {
		...
		Assert.assertTrue(addANewAddressPagePO.checkUserIsOnAddANewAddressPage());
	}
	...
	public void the_following_elements_are_present_on_the_Add_a_new_address_page(DataTable table) throws Throwable{
		addANewAddressPagePO = WC.getPageObject(AddANewAddressPagePO.class);
			...
			for (Map<String, String> myValue : mappedTable) {
				for (Map.Entry<String, String> entry : myValue.entrySet()) {
					...
					case "Breadcrumbs of the page":
						result = addANewAddressPagePO .checkBreadcrumbsIsDisplayed();
					...
					case "Postcode label":
						result = addANewAddressPagePO.checkPostcodeLabelIsDisplayed();
						Assert.assertTrue(result);
					...
					case "Evening phone textbox":
						result = addANewAddressPagePO .checkEveningPhoneTextboxIsDisplayed();
						Assert.assertTrue(result);
						LOG.debug("Evening phone textbox is displayed");
						break;
						
					case "Mobile phone label":
						result = addANewAddressPagePO .checkMobilePhoneLabelIsDisplayed();
						Assert.assertTrue(result);
						LOG.debug("Mobile phone label is displayed");
						break;
						
					case "Mobile phone textbox":
						result = addANewAddressPagePO .checkMobilePhoneTextboxIsDisplayed();
						Assert.assertTrue(result);
						LOG.debug("Mobile phone textbox| is displayed");
						break;
						
					case "Enabled Cancel Button":
						result = addANewAddressPagePO .checkEnabledCancelButtonIsDisplayed();
						Assert.assertTrue(result);
						LOG.debug("Enabled Cancel Button is displayed");
						break;
						
					case "Disabled Save address Button":
						result = addANewAddressPagePO .checkDisabledSaveAddressButtonIsDisplayed();
						Assert.assertTrue(result);
	...
	public void I_enter_the_following_details_for_the_Add_a_new_address_form(DataTable table) throws Throwable {
	...
	public void I_select_the_first_address_from_the_address_dropdown() throws Throwable {
	...
	public void the_Save_address_Button_is_enabled() throws Throwable {
		...
		Assert.assertTrue(addANewAddressPagePO.checkSaveAddressButtonIsEnabled());
	}
	...
	public void the_new_address_is_added() throws Throwable {
		...
		Assert.assertTrue(manageMyAddressBookPagePO.checkNewAddressIsDisplayed());
	}
